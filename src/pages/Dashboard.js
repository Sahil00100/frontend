...
function Dashboard(props) {
  ...
  const CreateEmployeeValidation = () => {
    ...
    if (!state.name) {
      ...
      isError = true;
      errorMessage = "phone_number is not provided!";
    }
  ...
  const createEmployee = async() =>{
    ...
    let {isError, errorMessage}  = CreateEmployeeValidation()
    ...
    if (isError){
      ...
      try {
        ...
        // setState({search:state.search,EmployeeList:[]})
        setfieldList([])
  ...
  const createCustomField = async() =>{
    ...
    try {
      ...
      setfieldList([])
      setState({search:state.search,EmployeeList:state.EmployeeList})
      set_settings(false)
      await fetchSettings()
    } catch (error) {
      console.error("Error creating :", error.response.data);
  ...
  const fetchData = async () => {
    try {
      ...
      if (EmployeeListresponse.status===200){
        ...
        setState({search:state.search,EmployeeList:data.data})
...
return (
  <div className="dashboard-main-container">
    ...
    <div className="main-div">
      {alignment === "Employee" ? (
        <EmployeeTable EmployeeList={state.EmployeeList} />
      ) : (
        <LineChartComponent />
      )}
    </div>
